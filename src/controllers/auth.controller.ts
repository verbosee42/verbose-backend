...
export async function registerProvider(req: Request, res: Response) {
  ...
  const { email, password, displayName, phone, state, city } = parsed.data;
  const emailNorm = normalizeEmail(email);
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (email, password_hash, role, display_name, phone)
       ...
       RETURNING id, email, role, display_name`,
      [emailNorm, passwordHash, displayName, phone ?? null]
    );

    const user = userResult.rows[0];

    // Create provider profile in PENDING verification
    const providerResult = await client.query(
      `INSERT INTO provider_profiles (user_id, display_name, state, city, verification_status)
       VALUES ($1, $2, $3, $4, 'PENDING')
       RETURNING id, verification_status, is_suspended, subscription_expires_at`,
      [user.id, displayName, state ?? null, city ?? null]
    );
...
export async function me(req: AuthedRequest, res: Response) {
  ...
  if (user.role === "PROVIDER") {
    const pRes = await pool.query(
      `SELECT id, display_name, state, city, verification_status, is_suspended, subscription_expires_at
 ...
 * - In real world: email the raw token link
 * MVP: return token in response (so you can test quickly), but REMOVE later.
 */
export async function forgotPassword(req: Request, res: Response) {
  ...
  // MVP: return token for testing. Replace with email sending.
